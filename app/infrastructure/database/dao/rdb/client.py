...
class ClientDAO(BaseDAO[Client]):
    ...
    async def create(
        self, branch_id: int, employee_id: int, client: schems.Client
    ) -> dto.Client:
        client = Client(**client.dict(), branch_id=branch_id, employee_id=employee_id)
    ...
    async def get_by_phone_number(
        self, phone_number: str, employee_id: int
    ) -> dto.Client:
        result = await self.session.execute(
            select(Client).where(
                Client.phone_number == phone_number, Client.employee_id == employee_id
            )
        )
    ...
    async def get_client(
        self, employee_id: int, client_id: int, branch_id: int
    ) -> dto.Client:
        result = await self.session.execute(
            select(Client).where(
                Client.id == client_id,
    ...
    async def update_appointment(
        self, client_id: int, employee_id: int, next_appointment_id: int
    ) -> dto.Client:
        result = await self.session.execute(
            update(Client)
            .where(Client.id == client_id, Client.employee_id == employee_id)
    ...
    async def update_client_period(
        self, client_id: int, employee_id: int, branch_id: int, period: int
    ) -> dto.Client:
        result = await self.session.execute(
            update(Client)
            .where(
    ...
    async def update_client(
        self, client_id: int, employee_id: int, client: schems.Client
    ) -> dto.Client:
        result = await self.session.execute(
            update(Client)
            .where(Client.id == client_id, Client.employee_id == employee_id)
